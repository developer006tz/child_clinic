@php $editing = isset($babyMedicalHostory) @endphp

<div class="row">
    <x-inputs.group class="col-sm-12">
        <x-inputs.select name="desease_id" label="Desease" required>
            @php $selected = old('desease_id', ($editing ? $babyMedicalHostory->desease_id : '')) @endphp
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.select name="level_of_illness" label="Level Of Illness">
            @php $selected = old('level_of_illness', ($editing ? $babyMedicalHostory->level_of_illness : '')) @endphp
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.textarea
            ...
            maxlength="255"
            required
            >{{ old('description', ($editing ? $babyMedicalHostory->description
            : '')) }}</x-inputs.textarea
        >
    </x-inputs.group>
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.date
            name="date"
            label="Date"
            value="{{ old('date', ($editing ? optional($babyMedicalHostory->date)->format('Y-m-d') : '')) }}"
            max="255"
            required
        ></x-inputs.date>
    </x-inputs.group>
